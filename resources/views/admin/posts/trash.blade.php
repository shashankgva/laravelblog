					...
					@if ($posts->count() > 0)
						@foreach( $posts as $post )
							<tr>
								...
								<td>
									<a href="{{ route('post.restore', ['id' => $post->id]) }}" class="btn btn-success btn-xs" title="">Restore</a>
								</td>
								<td>
									<a href="{{ route('post.kill', ['id' => $post->id]) }}" class="btn btn-success btn-xs" title="">Delete</a>
								</td>  
							</tr>
						@endforeach
					@else
